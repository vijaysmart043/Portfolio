    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 items-center gap-2 justify-center hidden sm:flex">
            {links.map((link) => (
              <HeaderLink
                key={link.title}
                title={link.title}
          ...
          <div className="flex items-center gap-2">
            <a
              href={"https://github.com/vijaysmart043"}
              target="_blank"
...
const HeaderLink = ({ title, href }: { title: string; href: string }) => {
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 px-2 py-1.5 rounded-full transition-colors text-sm md:text-base",
        isActive
